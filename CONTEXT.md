...
- **Cloudflare Pages (landing + docs)** — one project (`vars.CLOUDFLARE_PAGES_PROJECT`), two deployments via `wrangler pages deploy --branch <ref>`, serving only `index.html`, the svg, and the per-dataset schema docs (`schema.md/json/sql`, `README.md`) — static HTML needing no Range. It no longer carries `*.parquet` or `_files.json`. The deploy stages this content at the **deploy root**, so `index.html` sits at the root of the deployment. The public landing is **`https://petrodb.ocortez.com/`**, served directly by this Pages project's own custom domain — no router, no subpath, no redirect. petrodb's public hosting is deliberately independent of the SumpaLabs apex, which carries no petrodb tie (`sumpalabs-landing` ADR-0004), so petrodb takes no dependency on apex-zone infrastructure and no `sumpalabs.com` host is available to it. See [ADR-0007](docs/adr/0007-landing-hosts-independently-of-sumpalabs-apex.md), which supersedes [ADR-0006](docs/adr/0006-landing-at-sumpalabs-subpath.md) (its umbrella router was cancelled and never served live traffic).

`BASE_URL` (`scripts/transform/petrobras_3w/constants.py`) is the HF resolve base, per-branch (`vars.BASE_URL_MAIN` / `vars.BASE_URL_STAGE`); the `/petrobras_3w` dataset segment is appended by the constants module to match this dataset's directory under `parquet/`. It's per-branch because it's stamped into `instances.parquet#source_url` at build time and the catalog must reference the host where its own bytes are reachable. The deploy workflow runs on push to `main`/`stage` (push-to-`main` is the publish gate, no separate promote step), triggered by any change under `parquet/` or the 3W pipeline sources. It (re)builds the 3W tree — gated by a cache keyed on the 3W sources + the resolved `BASE_URL`, so a docs-only or non-3W change skips the rebuild — then **retargets every published data URL to the deploy's branch** (`scripts/deploy/retarget_data_urls.py`) and **uploads the data to HF** via the `hf` CLI (`--repo-type dataset --revision <branch>`, fine-grained `HF_TOKEN` secret; `--include '**/*.parquet' --include '**/_files.json'` so committed datasets ride along with the 3W tree) and runs a **slim Wrangler deploy** of the landing + docs. The retarget pass is a build-time host rewrite over the already-built artifacts (`index.html`, per-dataset schema `README.md`s, the root `README.md`, the HF dataset card): it rewrites the canonical committed host — the `main` HF resolve base, plus the dev Caddy host on the locally-generated 3W docs — to the deploy's `BASE_URL`, so a `stage` push ships only `/resolve/stage/` data URLs while the committed tree stays canonical `main` (never drifts per-branch). The 3W pipeline still bakes `BASE_URL` into its own surfaces first; the retarget covers the hand-authored Volve/FORCE tabs and the root-README quick-start that have no generator, needs no dataset sources (pure stdlib), and a CI grep guard fails the deploy if any off-branch data host survives (issue #42). See [ADR-0005](docs/adr/0005-host-parquet-on-huggingface.md) and [ADR-0003](docs/adr/0003-cloudflare-pages-hosting.md) (superseded).
